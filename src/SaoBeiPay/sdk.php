<?php
namespace xytool\PaySdk\SaoBeiPay;
...
class sdk extends Base
{
    /**
     * 公共参数
     * @var \xytool\PaySdk\SaoBeiPay\Params\PublicParams
     */
    public $publicParams;
    ...
    public function __parseExecuteData($params, &$data, &$requestData, &$url)
    {
      ...
      //  echo json_encode($data);exit;
        $sign_data = $this->array_sort($data,$data['make_sign_key']);//根据指定顺序排序
        $data['key_sign'] = $this->sign($sign_data);
        unset($data['_apiMethod']);
        unset($data['_method']);
        unset($data['apiDomain']);
        unset($data['sign_type']);
        unset($data['_isSyncVerify']);
        unset($data['appID']);
        unset($data['access_token']);
        unset($data['make_sign_key']);
        $requestData = $data;
    ...
    public function verifyCallback($data)
    {
        ...
        $content = $this->parseSignData($sign_data,true);
        switch($this->publicParams->sign_type)
        {
            case 'MD5':
    ...
    public function parseSignData($data,$null_flag=false)
    {
       ...
       // \ksort($data);
        ...
        $content .='access_token='.$this->publicParams->access_token;
        return trim($content, '&');
    }
